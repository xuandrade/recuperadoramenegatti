    ...
    <section id="pecas" className="relative py-28 md:py-36 border-t border-white/[0.05]">
      <div className="container-x">
        ...
        <div className="mt-16 grid lg:grid-cols-12 gap-6">
          <motion.ul initial="hidden" whileInView="show" viewport={{ once: true, margin: '-60px' }} variants={{ hidden: {}, show: { transition: { staggerChildren: 0.04 } } }} className="lg:col-span-7 grid sm:grid-cols-2 gap-2">
            {parts.map((p, i) => (
              <motion.li key={p} variants={{ hidden: { opacity: 0, x: -10 }, show: { opacity: 1, x: 0, transition: { duration: 0.6, ease } } }} className="group flex items-start gap-3 px-4 py-3.5 rounded-xl border border-white/[0.06] bg-white/[0.02] hover:bg-white/[0.04] hover:border-crimson-500/30 transition">
                <span className="mt-1 font-mono text-[10px] text-crimson-400/80 w-6 shrink-0">{String(i + 1).padStart(2, '0')}</span>
          ...
          <motion.div initial="hidden" whileInView="show" viewport={{ once: true, margin: '-60px' }} variants={{ hidden: {}, show: { transition: { staggerChildren: 0.1 } } }} className="lg:col-span-5 grid grid-cols-2 gap-3 auto-rows-[180px] md:auto-rows-[200px]">
            {showcase.map((g, i) => (
              <motion.figure key={g.title} variants={{ hidden: { opacity: 0, scale: 0.96 }, show: { opacity: 1, scale: 1, transition: { duration: 0.8, ease } } }} whileHover={{ y: -4 }} className={`rim group relative rounded-2xl overflow-hidden border border-white/10 ${i === 0 ? 'col-span-2 row-span-2' : ''}`}>
