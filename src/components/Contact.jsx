  ...
  return (
    <section id="contato" className="relative py-28 md:py-36 border-t border-white/[0.05]">
      <div className="container-x">
        <motion.div initial="hidden" whileInView="show" viewport={{ once: true }} variants={{ hidden: {}, show: { transition: { staggerChildren: 0.08 } } }} className="max-w-2xl">
        ...
        <div className="mt-14 grid lg:grid-cols-5 gap-6">
          <motion.form onSubmit={submit} initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.8, ease }} className="rim lg:col-span-3 glass-strong rounded-3xl p-6 md:p-8 shadow-luxe">
            ...
            <div className="mt-4">
              ...
              <textarea name="message" value={form.message} onChange={handle} rows={5} placeholder="Descreva a peça, o problema e prazos..." className="mt-2 w-full bg-white/[0.03] border border-white/10 focus:border-crimson-500/60 focus:bg-white/[0.05] rounded-xl px-4 py-3 outline-none transition placeholder:text-white/30" />
            </div>
          ...
          <motion.div initial="hidden" whileInView="show" viewport={{ once: true }} variants={{ hidden: {}, show: { transition: { staggerChildren: 0.08 } } }} className="lg:col-span-2 flex flex-col gap-4">
            {infos.map((i) => (
              <motion.a key={i.title} href={i.href || '#'} target={i.href ? '_blank' : undefined} rel="noreferrer" variants={{ hidden: { opacity: 0, x: 16 }, show: { opacity: 1, x: 0, transition: { duration: 0.7, ease } } }} whileHover={{ y: -3 }} className="rim glass rounded-2xl p-5 flex gap-4 hover:border-crimson-500/30 transition">
            ...
            <motion.div variants={{ hidden: { opacity: 0, scale: 0.97 }, show: { opacity: 1, scale: 1, transition: { duration: 0.8, ease } } }} className="rim glass rounded-2xl overflow-hidden h-56 relative">
              <iframe title="Mapa Recuperadora Menegatti" src="https://www.google.com/maps?q=Av.+dos+Democr%C3%A1ticos+515,+Higien%C3%B3polis,+Rio+de+Janeiro&output=embed" className="absolute inset-0 h-full w-full grayscale contrast-125 brightness-75" loading="lazy" />
...
function Field({ label, ...props }) {
  return (
    <div className="mt-4 sm:mt-0">
      ...
      <input {...props} className="mt-2 w-full bg-white/[0.03] border border-white/10 focus:border-crimson-500/60 focus:bg-white/[0.05] rounded-xl px-4 py-3 outline-none transition placeholder:text-white/30" />
    </div>
  );
}
