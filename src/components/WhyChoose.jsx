  ...
  return (
    <section id="diferenciais" className="relative py-28 md:py-36">
      ...
      <div className="container-x grid lg:grid-cols-2 gap-14 items-center">
        <motion.div initial="hidden" whileInView="show" viewport={{ once: true }} variants={{ hidden: {}, show: { transition: { staggerChildren: 0.08 } } }}>
          ...
          <motion.ul variants={{ hidden: {}, show: { transition: { staggerChildren: 0.06 } } }} className="mt-8 space-y-3">
            {checks.map((c) => (
              <motion.li key={c} variants={{ hidden: { opacity: 0, x: -10 }, show: { opacity: 1, x: 0, transition: { duration: 0.6, ease } } }} className="flex items-start gap-3">
        ...
        <motion.div initial="hidden" whileInView="show" viewport={{ once: true, margin: '-60px' }} variants={{ hidden: {}, show: { transition: { staggerChildren: 0.08 } } }} className="grid sm:grid-cols-2 gap-4">
          {reasons.map((r, i) => (
            <motion.div key={r.title} variants={{ hidden: { opacity: 0, y: 20 }, show: { opacity: 1, y: 0, transition: { duration: 0.7, ease } } }} whileHover={{ y: -4 }} className={`rim glass rounded-2xl p-6 ${i % 2 === 1 ? 'sm:translate-y-6' : ''}`}>
